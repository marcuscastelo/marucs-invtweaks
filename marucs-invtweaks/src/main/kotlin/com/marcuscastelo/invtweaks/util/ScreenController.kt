...
class ScreenController(private val handler: ScreenHandler) {
    ...
    fun dropOne(slot: Int) {
        ...
        if (isEmpty(slot)) {
            com.marcuscastelo.invtweaks.InvTweaksMod.LOGGER.warn("<ScreenController> Cannot dropOne stack, slot is empty!")
            //            return;
        }
    ...
    fun dropAll(slot: Int) {
        //TODO: check if held stack is empty?
        if (isEmpty(slot)) {
            com.marcuscastelo.invtweaks.InvTweaksMod.LOGGER.warn("<ScreenController> Cannot dropAll stack, slot is empty!")
            //            return;
        }
